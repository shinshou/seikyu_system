...
from .forms import InvoiceListingSearchForm, EstimateListingSearchForm, DeliveryListingSearchForm, ProductForm, ListingLinkSearchForm, ListingLinkShowSearchForm
from contracts.models import TraderPurchasesContract, TraderSalesContract, TraderLink, TraderContract, Milestone,InvoiceContract
...
class SalesListView(AdminLoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        search_form = InvoiceListingSearchForm(self.request.GET)
...
class EstimateListView(AdminLoginRequiredMixin, ListView):
    template_name = 'listing/estimates.html'
    context_object_name = 'products'
    paginate_by = 5

    def get_queryset(self):
        estimate_class_id = ContentType.objects.get(model='estimatecontract')
        queryset = ContractProduct.objects.prefetch_related('content_object').filter(
            Q(content_type=estimate_class_id)
        ).filter(available='T').order_by('-pk')
        
        search_form = EstimateListingSearchForm(self.request.GET)
        if search_form.is_valid():
            estimate_id = search_form.cleaned_data.get('estimate_id')
            created_at =  search_form.cleaned_data.get('created_at')
            created_at_to =  search_form.cleaned_data.get('created_at_to')
            issue_date_from =  search_form.cleaned_data.get('issue_date_from')
            issue_date_to =  search_form.cleaned_data.get('issue_date_to')
            customer = search_form.cleaned_data.get('customer')
            # name = search_form.cleaned_data.get('name')
            # person_in_charge = search_form.cleaned_data.get('person_in_charge')
            # inventory_status = search_form.cleaned_data.get('inventory_status')
            if estimate_id:
                queryset = queryset.filter(
                    Q(estimate_contract__estimate_id__icontains=estimate_id)
                )
            if created_at:
                queryset = queryset.filter(
                    Q(estimate_contract__created_at__gte=created_at)
                )

            if created_at_to:
                queryset = queryset.filter(
                    Q(estimate_contract__created_at__lte=created_at_to)
                )
            
            if issue_date_from:
                queryset = queryset.filter(
                    Q(estimate_contract__issue_date__gte=issue_date_from)
                )

            if issue_date_to:
                queryset = queryset.filter(
                    Q(estimate_contract__issue_date__lte=issue_date_to)
                )

            if customer:
                queryset = queryset.filter(
                    Q(estimate_contract__customer__name__icontains=customer)
                )

            # if person_in_charge:
            #     queryset = queryset.filter(
            #         Q(person_in_charge__icontains=person_in_charge)
            #     )
        ...
        # 同じcontruct product idのものは1つのまとめる
        grouped_results = defaultdict(list)
        for item in queryset:
            key = (item.object_id, item.content_type_id)
            grouped_results[key].append(item)

        # 各グループから最初のアイテムだけを選択する
        queryset = [items[0] for items in grouped_results.values()]

        return queryset
    
    def post(self, request, *args, **kwargs):
        user_id = self.request.user.id
        log_export_operation(user_id, "{} - {}".format(_("List"), _("Estimates")))
...
class InvoiceListView(AdminLoginRequiredMixin, ListView):
    template_name = 'listing/invoices.html'
    context_object_name = 'products'
    paginate_by = 5

    def get_queryset(self):
        invoice_class_id = ContentType.objects.get(model='invoicecontract')
        queryset = ContractProduct.objects.prefetch_related('content_object').filter(
            Q(content_type=invoice_class_id)
        ).filter(available='T').order_by('-pk')
        
        search_form = InvoiceListingSearchForm(self.request.GET)
        if search_form.is_valid():
            invoice_id = search_form.cleaned_data.get('invoice_id')
            created_at =  search_form.cleaned_data.get('created_at')
            created_at_to =  search_form.cleaned_data.get('created_at_to')
            billing_date_from =  search_form.cleaned_data.get('billing_date_from')
            billing_date_to =  search_form.cleaned_data.get('billing_date_to')
            customer = search_form.cleaned_data.get('customer')
            # name = search_form.cleaned_data.get('name')
            # person_in_charge = search_form.cleaned_data.get('person_in_charge')
            # inventory_status = search_form.cleaned_data.get('inventory_status')
            if invoice_id:
                queryset = queryset.filter(
                    Q(invoice_contract__invoice_id__icontains=invoice_id)
                )
            if created_at:
                queryset = queryset.filter(
                    Q(invoice_contract__created_at__gte=created_at)
                )

            if created_at_to:
                queryset = queryset.filter(
                    Q(invoice_contract__created_at__lte=created_at_to)
                )
            
            if billing_date_from:
                queryset = queryset.filter(
                    Q(invoice_contract__billing_date__gte=billing_date_from)
                )

            if billing_date_to:
                queryset = queryset.filter(
                    Q(invoice_contract__billing_date__lte=billing_date_to)
                )

            if customer:
                queryset = queryset.filter(
                    Q(invoice_contract__customer__name__icontains=customer)
                )
...
class DeliveryListView(AdminLoginRequiredMixin, ListView):
    template_name = 'listing/deliverys.html'
    context_object_name = 'products'
    paginate_by = 5

    def get_queryset(self):
        delivery_class_id = ContentType.objects.get(model='deliverycontract')
        queryset = ContractProduct.objects.prefetch_related('content_object').filter(
            Q(content_type=delivery_class_id)
        ).filter(available='T').order_by('-pk')
        
        search_form = DeliveryListingSearchForm(self.request.GET)
        if search_form.is_valid():
            delivery_id = search_form.cleaned_data.get('delivery_id')
            created_at =  search_form.cleaned_data.get('created_at')
            created_at_to =  search_form.cleaned_data.get('created_at_to')
            issue_date_from =  search_form.cleaned_data.get('issue_date_from')
            issue_date_to =  search_form.cleaned_data.get('issue_date_to')
            ...
            if delivery_id:
                queryset = queryset.filter(
                    Q(delivery_contract__delivery_id__icontains=delivery_id)
                )
            if created_at:
                queryset = queryset.filter(
                    Q(delivery_contract__created_at__gte=created_at)
                )

            if created_at_to:
                queryset = queryset.filter(
                    Q(delivery_contract__created_at__lte=created_at_to)
                )
            
            if issue_date_from:
                queryset = queryset.filter(
                    Q(delivery_contract__issue_date__gte=issue_date_from)
                )

            if issue_date_to:
                queryset = queryset.filter(
                    Q(delivery_contract__issue_date__lte=issue_date_to)
                )

            if customer:
                queryset = queryset.filter(
                    Q(delivery_contract__customer__name__icontains=customer)
                )
...
class EstimatesProductDetailAjaxView(AdminLoginRequiredMixin, View):
    def post(self, *args, **kwargs):
        if self.request.method == 'POST' and self.request.is_ajax():
            id = self.request.POST.get('id')
            product = ContractProduct.objects.get(id=id)
            contract = product.content_object
            contract_id = contract.invoice_id
            product_name = product.product.name
            # status = product.status
            return JsonResponse({
                'contract_id': contract_id,
...
class InvoicesProductDetailAjaxView(AdminLoginRequiredMixin, View):
    def post(self, *args, **kwargs):
        if self.request.method == 'POST' and self.request.is_ajax():
            id = self.request.POST.get('id')
            product = ContractProduct.objects.get(id=id)
...
class DeliverysProductDetailAjaxView(AdminLoginRequiredMixin, View):
    def post(self, *args, **kwargs):
        if self.request.method == 'POST' and self.request.is_ajax():
            id = self.request.POST.get('id')
            product = ContractProduct.objects.get(id=id)
