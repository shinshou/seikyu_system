...
class EstimateContract(models.Model):
    ...
    created_at = models.DateField() #見積書作成日
    customer = models.ForeignKey(Customer, on_delete=models.SET_NULL, null=True) #顧客
    issue_date = models.DateField() #請求日
    expiry_date = models.DateField(null=True, blank=True) #支払い期限日
    estimate_no = models.TextField() #見積書番号
    remarks = models.TextField(null=True, blank=True) #備考
    subject = models.TextField(null=True, blank=True) #件名
    fee = models.IntegerField(default=0) #手数料
    payment_method = models.CharField(max_length=2, choices=PAYMENT_METHOD_CHOICES) #支払い方法
    transfer_account = models.CharField(max_length=255, null=True, blank=True)
    person_in_charge = models.CharField(max_length=200, null=True, blank=True) #担当者
    total_price = models.IntegerField(default=0) #総額
    own_company_name = models.TextField(null=True, blank=True) #自社名
    own_company_postal_code = models.TextField(null=True, blank=True) #自社郵便番号
    own_company_address = models.TextField(null=True, blank=True) #自社住所
    own_company_tel = models.TextField(null=True, blank=True) #自社電話番号
    own_company_mailaddress = models.TextField(null=True, blank=True) #自社メールアドレス
    own_company_invoice_no = models.TextField(null=True, blank=True) #自社インボイス番号
    products = GenericRelation(ContractProduct, related_query_name='estimate_contract') #商品
    documents = GenericRelation(ContractDocument, related_query_name='estimate_contract')
...
class InvoiceContract(models.Model):
    ...
    created_at = models.DateField() #請求書作成日
    customer = models.ForeignKey(Customer, on_delete=models.SET_NULL, null=True) #顧客
    billing_date = models.DateField() #請求日
    payment_deadline = models.DateField(null=True, blank=True) #支払い期限日
    billing_no = models.TextField() #請求番号
    remarks = models.TextField(null=True, blank=True) #備考
    subject = models.TextField(null=True, blank=True) #件名
    fee = models.IntegerField(default=0) #手数料
    payment_method = models.CharField(max_length=2, choices=PAYMENT_METHOD_CHOICES) #支払い方法
    ...
    own_company_mailaddress = models.TextField(null=True, blank=True) #自社メールアドレス
    own_company_invoice_no = models.TextField(null=True, blank=True) #自社インボイス番号
    products = GenericRelation(ContractProduct, related_query_name='invoice_contract') #商品
    documents = GenericRelation(ContractDocument, related_query_name='invoice_contract')
    payment_link = models.CharField(max_length=255, null=True, blank=True) # 支払いリンク
...
class DeliveryContract(models.Model):
    ...
    delivery_no = models.TextField() #見積書番号
    remarks = models.TextField(null=True, blank=True) #備考
    subject = models.TextField(null=True, blank=True) #件名
    fee = models.IntegerField(default=0) #手数料
    payment_method = models.CharField(max_length=2, choices=PAYMENT_METHOD_CHOICES) #支払い方法
    ...
    own_company_mailaddress = models.TextField(null=True, blank=True) #自社メールアドレス
    own_company_invoice_no = models.TextField(null=True, blank=True) #自社インボイス番号
    products = GenericRelation(ContractProduct, related_query_name='delivery_contract') #商品
    documents = GenericRelation(ContractDocument, related_query_name='delivery_contract')
