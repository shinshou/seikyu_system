...
from masterdata.models import Sender, Document, DocumentFee, PersonInCharge,InventoryProduct
from .models import (
    ...
    InvoiceContract, EstimateContract, DeliveryContract, Customer,
)
from .forms import (
...
class EstimateContractUpdateView(AdminLoginRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        id = kwargs.get('pk')
        contract_form = EstimateContractForm(self.request.POST, id=id)
        # mdate = self.request.POST.dict()['milestone-0-date']
        # try: 
        #     try:
        #         mdate = datetime.strptime(mdate, '%m/%d/%Y')
        ...
        product_formset = ProductFormSet(
            self.request.POST,
            form_kwargs={'invoice_id': contract.id, 'invoice_class': 'EstimateContract'},
            prefix='product'
        )
        if product_formset.is_valid():
            product_qs = contract.products.values('id')
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        id = kwargs.get('pk')
        contract = EstimateContract.objects.get(id=id)
        contract_data = {
            'estimate_id': contract.estimate_id,
            'created_at': contract.created_at,
            'customer_id': contract.customer.id if contract.customer.id else None,
            'customer_name': contract.customer.name if contract.customer.name else None,
        ...
        context['estimate_form'] = EstimateContractForm(initial=contract_data)
        ...
        context['update_estimate_id'] = contract.estimate_id if contract.estimate_id else None
        context['documents'] = Document.objects.all().values('id', 'name')
        document_fee_lambda = lambda df: {'id': df.id, 'name': df.get_type_display()}
        document_fees = [document_fee_lambda(document_fee) for document_fee in DocumentFee.objects.all()]
        context['document_fees'] = document_fees
...
class InvoiceContractUpdateView(AdminLoginRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        id = kwargs.get('pk')
        contract_form = InvoiceContractForm(self.request.POST, id=id)
        # mdate = self.request.POST.dict()['milestone-0-date']
        # try: 
        #     try:
        #         mdate = datetime.strptime(mdate, '%m/%d/%Y')
        ...
        product_formset = ProductFormSet(
            self.request.POST,
            form_kwargs={'invoice_id': contract.id, 'invoice_class': 'InvoiceContract'},
            prefix='product'
        )
        if product_formset.is_valid():
            product_qs = contract.products.values('id')
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        id = kwargs.get('pk')
        contract = InvoiceContract.objects.get(id=id)
        contract_data = {
        ...
        context['invoice_form'] = InvoiceContractForm(initial=contract_data)
        context['customers'] = Customer.objects.all().values('id', 'name','frigana', 'address', 'tel','mailaddress', 'payee')
        context['products'] = InventoryProduct.objects.filter(quantity__gt=0).values('id', 'product_id', 'product__name', 'quantity', 'price')
        context['update_invoice_id'] = contract.invoice_id if contract.invoice_id else None
        context['documents'] = Document.objects.all().values('id', 'name')
        document_fee_lambda = lambda df: {'id': df.id, 'name': df.get_type_display()}
        document_fees = [document_fee_lambda(document_fee) for document_fee in DocumentFee.objects.all()]
        context['document_fees'] = document_fees
...
class DeliveryContractUpdateView(AdminLoginRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        id = kwargs.get('pk')
        contract_form = DeliveryContractForm(self.request.POST, id=id)
        # mdate = self.request.POST.dict()['milestone-0-date']
        # try: 
        #     try:
        #         mdate = datetime.strptime(mdate, '%m/%d/%Y')
        ...
        product_formset = ProductFormSet(
            self.request.POST,
            form_kwargs={'invoice_id': contract.id, 'invoice_class': 'DeliveryContract'},
            prefix='product'
        )
        if product_formset.is_valid():
            product_qs = contract.products.values('id')
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        id = kwargs.get('pk')
        contract = DeliveryContract.objects.get(id=id)
        contract_data = {
        ...
        context['delivery_form'] = DeliveryContractForm(initial=contract_data)
        context['customers'] = Customer.objects.all().values('id', 'name','frigana', 'address', 'tel','mailaddress', 'payee')
        context['products'] = InventoryProduct.objects.filter(quantity__gt=0).values('id', 'product_id', 'product__name', 'quantity', 'price')
        context['update_delivery_id'] = contract.delivery_id if contract.delivery_id else None
        context['documents'] = Document.objects.all().values('id', 'name')
        document_fee_lambda = lambda df: {'id': df.id, 'name': df.get_type_display()}
        document_fees = [document_fee_lambda(document_fee) for document_fee in DocumentFee.objects.all()]
        context['document_fees'] = document_fees
