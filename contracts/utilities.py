...
def generate_invoice_id(prefix='01'):
    year = time.strftime("%y", time.localtime())
    month = time.strftime("%m", time.localtime())
    day = time.strftime("%d", time.localtime())
    milisec = int(datetime.now().strftime('%f')) + r.randint(0, 999999999)
    invoice_id = prefix + year + month + day
    count = InvoiceContract.objects.count()
    count += EstimateContract.objects.count()
    count += DeliveryContract.objects.count()
    invoice_id += str(count + 1).zfill(5)
    invoice_id = str(int(invoice_id) + int(milisec))
    return invoice_id
