...
from .filters import CustomerFilter, HallFilter, SenderFilter, ProductFilter, InventoryProductFilter
import json
import datetime
...
class CustomerListView(AdminLoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return CustomerFilter(self.request.GET, queryset=self.queryset).qs.order_by(
            "pk"
        )
...
class CustomerUpdateView(AdminLoginRequiredMixin, View):
    def post(self, *args, **kwargs):
        ...
        sender = Sender.objects.filter(
            customer_name=customer.customer_name, address=customer.address
        ).first()
        ...
        customer.mailaddress = data["mailaddress"]
        customer.payee = data["payee"]
        customer.save()
        ...
        sender.tel = data["tel"]
        sender.mailaddress = data["mailaddress"]
        sender.payee = data["payee"]
        sender.save()
...
class HallDeleteEvaluateAjaxView(AdminLoginRequiredMixin, View):
    def post(self, *args, **kwargs):
        if self.request.method == "POST" and self.request.is_ajax():
            ...
            hall = Hall.objects.get(id=id)
            contract_hall = (
                HallSalesContract.objects.filter(hall=hall).first()
                or HallPurchasesContract.objects.filter(hall=hall).first()
            )
...
class InventoryProductListView(AdminLoginRequiredMixin, ListView):
    template_name = "inventory/inventory_products.html"
    queryset = InventoryProduct.objects.all()
    context_object_name = "inventory_products"
    paginate_by = 10

    def get_queryset(self):
        return InventoryProductFilter(self.request.GET, queryset=self.queryset).qs.order_by("pk")

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["inventory_product_filter"] = InventoryProductFilter(self.request.GET)
        context["people"] = PersonInCharge.objects.all()
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            form.save()
        return redirect("masterdata:inventory-product-list")
    
class InventoryProductUpdateView(AdminLoginRequiredMixin, View):
    def post(self, *args, **kwargs):
        id = self.request.POST.get("id")
        inventory_product = InventoryProduct.objects.get(id=id)
        inventory_product_form = InventoryProductForm(self.request.POST)
        if inventory_product_form.is_valid():
            now = datetime.datetime.now()

            data = inventory_product_form.cleaned_data
            inventory_product.quantity = data.get("quantity")
            inventory_product.updated_date = now
...
class InventoryProductDetailAjaxView(AdminLoginRequiredMixin, View):
    def post(self, *args, **kwargs):
        if self.request.method == "POST" and self.request.is_ajax():
            ...
            return JsonResponse(
                {
                    "name": inventory_product.product.name,
                    "category": inventory_product.product.category,
                    "brand": inventory_product.product.brand,
                    "updated_date": inventory_product.updated_date,
    ...
    def get(self, *args, **kwargs):
        if self.request.method == "GET" and self.request.is_ajax():
            ...
            customer_qs = customer_qs.order_by("id")[start:end].values(
                "id", "name", "frigana", "tel", "postal_code", "address"
            )
            customers = list(customer_qs)
            return JsonResponse(
                {"customers": customers, "total_count": total_count},
                safe=False,
                status=200,
            )
        return JsonResponse({"success": False}, status=400)
...
class HallSearchAjaxView(AdminLoginRequiredMixin, View):
    def get(self, *args, **kwargs):
        if self.request.method == "GET" and self.request.is_ajax():
            ...
            halls = list(hall_qs)
            return JsonResponse(
                {"halls": halls, "total_count": total_count}, safe=False, status=200
            )
        return JsonResponse({"success": False}, status=400)
...
class SenderSearchAjaxView(AdminLoginRequiredMixin, View):
    def get(self, *args, **kwargs):
        if self.request.method == "GET" and self.request.is_ajax():
            ...
            return JsonResponse(
                {"senders": senders, "total_count": total_count}, safe=False, status=200
            )
        return JsonResponse({"success": False}, status=400)
...
class ProductSearchAjaxView(AdminLoginRequiredMixin, View):
    def get(self, *args, **kwargs):
        if self.request.method == "GET" and self.request.is_ajax():
            ...
            return JsonResponse(
                {"products": products, "total_count": total_count},
                safe=False,
                status=200,
            )
        return JsonResponse({"success": False}, status=400)
